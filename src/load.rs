...
impl Load {
    ...
    fn load_certs(&self, load: &str) -> Vec<rustls::Certificate> {
        // // let certfile = File::open(filename).expect("cannot open certificate file");
        // let mut reader = BufReader::new(load.as_bytes());
        // rustls_pemfile::certs(&mut reader)
        //     .unwrap()
        //     .iter()
        //     .map(|v| rustls::Certificate(v.clone()))
        //     .collect()
        vec![rustls::Certificate(load.as_bytes().to_vec())]
    }

    fn load_private_key(&self, load: &str) -> rustls::PrivateKey {
        let buf = load.as_bytes().to_vec();
        rustls::PrivateKey(buf)
    }
    ...
    pub fn configure_client(&self) -> Arc<rustls::ClientConfig> {
        let cert_file = self.to_ca_vec();
        let mut root_store = RootCertStore::empty();
        root_store.add_parsable_certificates(&[cert_file]);
